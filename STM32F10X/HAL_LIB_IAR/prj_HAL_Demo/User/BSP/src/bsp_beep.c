...
//#define BEEP_HAVE_POWER		/* 定义此行表示有源蜂鸣器，直接通过GPIO驱动, 无需PWM */
	...
	#define BEEP_DISABLE()	GPIO_PORT_BEEP->BSRR = ((uint32_t)GPIO_PIN_BEEP)<<16			/* 禁止蜂鸣器鸣叫 */
#else		/* 无源蜂鸣器 */
	/* PA8 ---> TIM1_CH1 */

	/* 1500表示频率1.5KHz，5000表示50.00%的占空比 */
	#define BEEP_ENABLE()	bsp_SetTIMOutPWM(GPIOB, GPIO_PIN_7, TIM4, 2, 1500, 5000);

	/* 禁止蜂鸣器鸣叫 */
	#define BEEP_DISABLE()	bsp_SetTIMOutPWM(GPIOB, GPIO_PIN_7, TIM4, 2, 1500, 0);
#endif
...
void BEEP_InitHard(void)
{
	...
	BEEP_DISABLE();
	bsp_ConfigGpioOut(GPIO_PORT_BEEP, GPIO_PIN_BEEP);	/* 配置GPIO为推挽输出 */	
